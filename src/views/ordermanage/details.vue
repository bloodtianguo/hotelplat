...
import { orderconfirm, orderocc, orderout,refundOrderById,orderrefund } from "@/api/order/ordermanage";
export default {
  created() {
    if (sessionStorage.orderid) {
      this.searchdata.id = sessionStorage.orderid;
      this.confirmdata.id = sessionStorage.orderid;
    }
    this.fetchData();
  },
  data() {
    return {
      ...
      giftList:[
        { pro: "spreeName", labels: "礼包名称" },
        { pro: "spreeTitle", labels: "副标题" },
      ],
      spreeIcon:"",
      searchdata: {
        id: "",
  ...
  methods: {
    ...
    refundOk() {
      ...
      this.refundFun("是否同意该退款请求！！");
      this.fetchData()
    },
    //拒绝退款
    refundNo() {
      this.confirmdata.pass = false;
      this.refundFun("是否拒绝该退款请求！！");
      this.fetchData()
    },
    fetchData() {
