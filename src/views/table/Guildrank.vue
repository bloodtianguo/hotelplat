...
<script>
import { guildRank } from '@/api/guild/guildrank'
...
export default {
  ...
  data() {
    return {
      ...
      // monthUp	number 月涨幅
      // ranges	string 上月涨幅
      // rank	integer ($int32) 排名
      // totalJBalance	number 找票打赏
      // upOrDown	integer ($int32) 跌或涨 0-涨 1-跌
      tableData: []
    }
  },
  created() {
  ...
  methods: {
    fetchData() {
      ...
      this.loading = true
      guildRank(this.searchData).then(response => {
        if (response.code == "0") {
